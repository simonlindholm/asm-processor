...
Tests need the environment variable `MIPS_CC` set as the IDO 7.1 compiler.
...
```sh
MIPS_CC=../oot/tools/ido_recomp/linux/7.1/cc ./run-tests.sh
```

Or using [qemu-irix](https://github.com/zeldaret/oot/releases/tag/0.1q) (don't forget `chmod u+x qemu-irix`) to emulate IDO:

```sh
MIPS_CC='./qemu-irix -silent -L ../oot/tools/ido7.1_compiler/ ../oot/tools/ido7.1_compiler/usr/bin/cc' ./run-tests.sh
```
